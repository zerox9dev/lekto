      ...
      <div className="space-y-3">
        {qs.map((q, qi) => (
          <div key={qi} className="space-y-1.5">
            ...
            {q.explanation && <p className="text-[11px] text-[#888] pl-2 italic">Пояснение: {q.explanation}</p>}
          </div>
        ))}
      </div>
    );
              ...
              {homeworkItems.map((h) => {
                ...
                return (
                  <div key={h.id} className="rounded-lg border border-[#e8e5de] px-3 py-2.5 flex items-center gap-2 md:gap-3 group/hw">
                    {h.completed ? (
                      <div className="h-5 w-5 rounded-md bg-[#2d5a3d] flex items-center justify-center shrink-0">
                        <Check className="h-3 w-3 text-white" />
                      </div>
                    ) : <Circle className="h-4 w-4 text-[#ccc] shrink-0" />}
                    <p className="text-[12px] font-medium truncate flex-1">{h.title}</p>
                    {avgScore !== null && (
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-[#ddefd7] text-[#2d5a3d] shrink-0">{avgScore}%</span>
                    )}
